...
std::string getTestConnectionString() {
    ...
    const std::string db =
        getEnvOrDefault("POSTGRES_DB", "sensors") + "_test";
...
protected:
    ...
    void SetUp() override {
        ...
        repository =
            std::make_shared<SensorRepository>(
                connectionString
            );
        ...
        try {
            ...
            std::cerr
                << "Warning: Could not truncate test table: "
                << e.what()
                << std::endl;

            std::cerr
                << "Make sure the test database exists, "
                << "the schema has been applied, and PostgreSQL "
                << "is accessible."
                << std::endl;
        }
    }
...
TEST_F(
    SensorRepositoryIntegrationTest,
    InsertAndGetById
) {
    SensorDataCreate createDto{
        "test_sensor_1",
        23.5,
        "Celsius"
    };
...
TEST_F(
    SensorRepositoryIntegrationTest,
    GetAllReturnsInsertedItems
) {
    SensorDataCreate dto1{
        "test_sensor_2",
        10.0,
        "Volt"
    };

    SensorDataCreate dto2{
        "test_sensor_3",
        15.5,
        "Ampere"
    };
    ...
    for (const auto& sensor : all) {
        ...
        if (
            sensor.sensor_id == "test_sensor_2" &&
        ...
        if (
            sensor.sensor_id == "test_sensor_3" &&
